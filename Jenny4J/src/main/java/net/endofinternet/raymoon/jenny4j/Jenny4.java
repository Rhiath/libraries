...
public class Jenny4 {
    ...
    public Jenny4(ConstraintChecker checker, List<Domain> domains) {
    ...
    public List<Combination> solve(int n) {
        ...
        Map<Combination, List<Combination>> bruteForceByNPair;// = buildBruteForceLookup(bruteForce, uncoveredCombinations);
        ...
        while (!uncoveredCombinations.isEmpty() && !bruteForce.isEmpty()) {
            ...
            bruteForceByNPair = BruteForceLookupBuilder3.buildBruteForceLookup(bruteForce, uncoveredCombinations, n);
            ...
            for (int i = 0; i < 10 && !uncoveredCombinations.isEmpty(); i++) {

                Combination nextBestNPair = uncoveredCombinations.get(0);
                List<Combination> bruteForceSublist = bruteForceByNPair.get(nextBestNPair);
//                System.out.println("there are " + bruteForceSublist.size() + " combinations for the NPair that matches the most combinations");
                ...
                Combination nextBest = bruteForceSublist.get(0);
                ...
                if (matchedCombinations.size() > 0) {
                    ...
                    for (Combination matchesNPair : matchedCombinations) {
                        bruteForceByNPair.remove(matchesNPair);
                    }
...
//    }
    ...
    private Comparator<Combination> sortByNumberOfMatches(final Map<Combination, List<Combination>> bruteForceByNPair) {
        return new Comparator<Combination>() {
            public int compare(Combination o1, Combination o2) {
